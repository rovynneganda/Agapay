...
const AdminReports = () => {
  ...
  const handleSendResponder = (reportId) => {
    setReportId(reportId);
    for (const [rescueType, rescueValue] of Object.entries(rescue)) {
      const formData = new FormData();
      formData.append("fileSelector", "gis");
      formData.append("reportId", reportId);
      formData.append("rescueType", rescueType);
      formData.append("rescueTypeValue", rescueValue);
  ...
  const handleDropdownChange = (event, dropdownName, rescueType, id) => {
    ...
    // For example, you can use React state to store the selected values
                    ...
                    <div>
                      ...
                      {report.disaster === "Flood" && (
                        <div className="mt-4 space-y-3 mb-5">
                          ...
                          <select
                            className="bg-subtlegray border border-gray/30 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 font-inter"
                            onChange={(event) => handleDropdownChange(event, "Rescue_1", "Rescue Team", report.id)}
                          >
                          ...
                          <select
                            className="bg-subtlegray border border-gray/30 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 font-inter"
                            onChange={(event) => handleDropdownChange(event, "Rescue_2", "Ambulance", report.id)}
                          >
                            {/* You can customize the options based on your requirements */}
                      ...
                      {report.disaster === "Accident" && (
                        <div className="mt-4 space-y-3 mb-5">
                          <select
                            id={`responders-${index}`}
                            className="bg-subtlegray border border-gray/30 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 font-inter"
                            onChange={(event) => handleDropdownChange(event, "Rescue_1", "Ambulance", report.id)}
                          >
                            <option value="0">No Ambulance</option>
                            <option value="1">1 Ambulance</option>
                            <option value="2">2 Ambulance</option>
                          ...
                          <select
                            id={`responders-${index}`}
                            className="bg-subtlegray border border-gray/30 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 font-inter"
                            onChange={(event) => handleDropdownChange(event, "Rescue_2", "Police Department", report.id)}
                          >
                      ...
                      {report.disaster === "Fire" && (
                        <div className="mt-4 space-y-3 mb-5">
                          <select
                            ...
                            // onChange={}
                            className="bg-subtlegray border border-gray/30 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 font-inter"
                            onChange={(event) => handleDropdownChange(event, "Rescue_1", "Fire Department", report.id)}
                          >
                      ...
                      <div className="flex flex-row gap-3 justify-center mt-2">
                        <button
                          ...
                          // onClick={handleSendResponder(report.id)}
                          onClick={() => handleSendResponder(report.id)}
                        >
                          Send Responder
                        </button>

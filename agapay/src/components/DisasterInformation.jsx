...
const DisasterInformation = () => {
  ...
  const selectedCard = disasterCards.find((card) => card.title.toLowerCase().replace(/\s/g, '-') === formattedTitle);

  if (!selectedCard) {
    return <div>Card not found</div>;
  }

  const { beforetitle, beforedescription1, beforedescription2, beforedescription3, beforedescription4, duringdescription1, duringdescription2, duringdescription3, duringdescription4, afterdescription1, afterdescription2, afterdescription3, afterdescription4 } = selectedCard;

  return (
   <>
   <header className='mt-40 p-5'>
    <Link to="/safety-reminders" >
    <button className=' mb-5   py-2 px-2 rounded-xl font-poppins text-black hover:text-white font-semibold text-sm hover:bg-primarydark'><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 inline-block mr-3">
